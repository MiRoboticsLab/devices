...
public:
    bool Init() override
    {
        ...
        //初始化toml实例
        RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), "COMAMAND_OFF: %ld",protocol::srv::LedExecute::Request::COMMAND_OFF);
        RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), "COMAMAND_OFF: %lu",(protocol::srv::LedExecute::Request::COMMAND_ALWAYSON));
    ...
    void Play(const std::shared_ptr<protocol::srv::LedExecute::Request> info_request,
...
//void LedCarpo::start_led_thread(LedCarpo* temp_ptr)
//{
  //  temp_ptr->tail_led_thread();
//}
...
void LedCarpo::tail_led_thread()
{
    ...
    std::unique_lock<std::mutex> run_tail_led_lock(tail_led_run_mutex);
    cyberdog::common::Protocol<TailLed> tail_led_can("/home/mi/workspace/cyberdog_ws/devices/cyberdog_led/tail_led.toml",true);
    tail_led_can.LINK_VAR(tail_led_can.GetData()->effect_id);
    std::shared_ptr tail_led_data = tail_led_can.GetData();
    while (!ready)
    {
        
        tail_led_waitcv.wait(run_tail_led_lock,[&] {
        ...
        if (tail_led_workable==true)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), "began run tail_led_thread !! request:  client: %s" ", target: %d" ", priority: %d"  ", timeout: %lu" ", effect: %lu",
                (new_request.client).c_str(), new_request.target, new_request.priority,new_request.timeout,new_request.effect);
        ...
        {
        RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), "new  tail_led_thread request:  client: %s" ", target: %d" ", priority: %d"  ", timeout: %lu" ", effect: %lu" ".  run over!!!",
                (new_request.client).c_str(), new_request.target, new_request.priority,new_request.timeout,new_request.effect);
            //继续执行上一条长亮指令
            tail_led_data->effect_id= tail_led_alwayson_id;
            RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), " tail_led_alwayson_i=" " : %d",tail_led_alwayson_id);

            tail_led_can.SendSelfData();
            RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), " alwayson tail_led_thread comand running ,tail_led_thread  dump again! ");
...
void LedCarpo::mini_led_thread()
{
    ...
    std::unique_lock<std::mutex> run_mini_led_lock(mini_led_run_mutex);
    cyberdog::common::Protocol<MiniLed> mini_led_can("/home/mi/workspace/cyberdog_ws/devices/cyberdog_led/mini_led.toml",true);
    mini_led_can.LINK_VAR(mini_led_can.GetData()->effect_id);
    std::shared_ptr mini_led_data = mini_led_can.GetData();
    while (!ready)
    {
        
        mini_led_waitcv.wait(run_mini_led_lock,[&] {
        ...
        if (mini_led_workable==true)
        {
            ...
            RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), "began run mini_led_thread !! request:  client: %s" ", target: %d" ", priority: %d"  ", timeout: %lu" ", effect: %lu",
                (new_request.client).c_str(), new_request.target, new_request.priority,new_request.timeout,new_request.effect);
        ...
        {
        RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), "new  mini_led_thread request:  client: %s" ", target: %d" ", priority: %d"  ", timeout: %lu" ", effect: %lu" ".  run over!!!",
                (new_request.client).c_str(), new_request.target, new_request.priority,new_request.timeout,new_request.effect);
            //继续执行上一条长亮指令
            mini_led_data->effect_id= mini_led_alwayson_id;
            mini_led_can.SendSelfData();
            RCLCPP_INFO(rclcpp::get_logger("cyberdog_led"), " alwayson mini_led_thread comand running ,mini_led_thread  dump again! ");
...
//bool LedCarpo::command_available()
//{
  //  tail_led_workable!=false;
//}
